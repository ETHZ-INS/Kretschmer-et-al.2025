...
#' Performs an Novel object recognition  analysis on an object of type TrackingData
#' 
#' @param t an object of type TrackingData
#' @param movement_cutoff a numeric value that denotes the cutoff point above which an animal is considered moving
#' @param integration_period a numeric value that denotes the duration over which metrics are smoothed.
#' @param points a string or vector of strings that denotes the name of points which will be analysed
#' @param interaction_distance a numeric value. defines below what value we recognize an interaction of nose to the two objects
#' @return a TrackingData object
#' @examples
#' OFTAnalysis(t, movement_cutoff = 5,integration_period = 5,points = "bodycentre", interaction_distance = 3)
NORAnalysis <- function(t, movement_cutoff,integration_period, points, interaction_distance){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  if(is.null(t$zones)){
  ...
  distance_object1 <- GetDistances(t,"nose","pos2")
  distance_object2 <- GetDistances(t,"nose","pos3")
  distance_object1 <- distance_object1 < interaction_distance
  distance_object2 <- distance_object2 < interaction_distance
  ...
  distance_object1 <- GetDistances(t,"nose","pos2")
  distance_object2 <- GetDistances(t,"nose","pos3")
  t$labels[["interaction.pos2"]] <- distance_object1 < interaction_distance
  t$labels[["interaction.pos3"]] <- distance_object2 < interaction_distance
...
#' Compiles a list of zone entries and exits for an object of type Trackingdata
#' 
#' @param t an object of type TrackingData
#' @param p string value of a point
#' @param z a string or vector of strings naming the zones to be checked
#' @return a data.frame of zone entries with columns for filename, zonename, onset and offset
#' @examples
#' GetZoneEntries(t, "bodycentre",c("center","periphery))
GetZoneEntries <- function(t,p,z){
  out <- data.frame()
  ...
  for(i in z){
    ...
    #calculate onsets and offsets of zone entries
    ons <- t$frames[which(!inz[1:(length(inz) -1)] & inz[2:(length(inz))])]
    offs <- t$frames[which(inz[1:(length(inz) -1)] & !inz[2:(length(inz))])]

    #fix special cases where animal starts or ends session in a zone
    if(inz[1]){
      ons <- append(t$frames[1],ons)
    }
    if(inz[length(inz)]){
      offs <- append(offs, t$frames[length(inz)])
    }
    out <- rbind(out, data.frame(recording_name = t$filename, zone = i, start_points = ons, end_points = offs))
  }
  out <- out[order(out$start_points),]
  return(out)
}
